...
// ─── GENERATE RADIOLOGY REPORT PDF (Simplified Working Version) ───
function generateReportPDF(report) {
  return new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Patient Name:', leftCol, currentY);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.patientName || '—', leftCol + 85, currentY);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Patient ID:', leftCol, currentY + 20);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.patientId || '—', leftCol + 85, currentY + 20);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Date of Birth:', leftCol, currentY + 40);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.dob || '—', leftCol + 85, currentY + 40);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Sex:', leftCol, currentY + 60);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.sex || '—', leftCol + 85, currentY + 60);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Accession #:', rightCol, currentY);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.accessionNumber || '—', rightCol + 85, currentY);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Modality:', rightCol, currentY + 20);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.modality || '—', rightCol + 85, currentY + 20);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Study Date:', rightCol, currentY + 40);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.studyDate || '—', rightCol + 85, currentY + 40);
    
    doc.font('Helvetica-Bold').fontSize(9).fillColor('#4A5568')
       .text('Referring Physician:', rightCol, currentY + 60);
    doc.font('Helvetica').fontSize(10).fillColor('#1A202C')
       .text(report.referringPhysician || '—', rightCol + 85, currentY + 60);
    ...
    function addSection(title, content) {
      ...
      doc.font('Helvetica').fontSize(10).fillColor('#2D3748')
         .text(content, 50, doc.y, { width: doc.page.width - 100, lineGap: 4 });
      doc.moveDown(1);
    }
    ...
    doc.font('Helvetica').fontSize(9).fillColor('#4A5568')
       .text('Radiologist Signature:', sigX, sigY);
    
    doc.strokeColor('#2D3748').lineWidth(0.8)
       .moveTo(sigX, sigY + 15).lineTo(sigX + 150, sigY + 15).stroke();
    
    doc.font('Helvetica-Bold').fontSize(10).fillColor('#2D3748')
       .text(report.radiologist || '_________________________', sigX, sigY + 20, {
    ...
    doc.font('Helvetica').fontSize(8).fillColor('#A0AEC0')
       .text(`Page 1 of ${pageCount}`, 50, doc.page.height - 40, 
